...
# Injeção de CSS para garantir a centralização das tabelas nativas do Streamlit
st.markdown("""
    <style>
...
# --- LÓGICA DE ORDENAÇÃO DINÂMICA DOS GRÁFICOS ---
# 1. Agrupa as classes pelo valor real
df_resumo_classe = df.groupby('Classe')['Total Atual'].sum().reset_index()

# Ordena do maior patrimônio para o menor para a Legenda e Pílulas
df_resumo_classe_desc = df_resumo_classe.sort_values(by='Total Atual', ascending=False)

# Para o gráfico em si rodar perfeitamente para a esquerda (anti-horário) começando do topo,
# os dados precisam entrar na ordem decrescente exata.
df_classes_grafico = df_resumo_classe_desc.copy()

# Vincula a ordem geográfica das classes para ordenar os ativos internamente por classe e por tamanho
lista_classes_ordenada = df_classes_grafico['Classe'].tolist()
...
# Tabela detalhe clássica (Sempre decrescente por patrimônio)
...
with aba_dash:
    ...
    with col1:
        ...
        # rotation=90 + counterclockwise + sort=False com os dados ordenados decrescentes força a maior fatia no 2º Quadrante
        fig_classe.update_traces(rotation=90, direction='counterclockwise', textinfo='percent+label', sort=False)
        # Garante a legenda lateral ordenada do maior para o menor
