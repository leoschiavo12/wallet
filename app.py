...
def obter_precos_b3(tickers_lista):
    ...
    try:
        ...
        for t in tickers_lista:
            chave = f"{t.upper()}.SA"
            try:
                preco = dados[chave]['Close'].ffill().iloc[-1] if len(tickers_formatados) > 1 else dados['Close'].ffill().iloc[-1]
...
for classe, ativos in MINHA_CARTEIRA.items():
    for ticker, qtd in ativos.items():
        if classe in ['ETF', 'FII']: preco = bancada_precos.get(ticker.upper(), 0.0)
        elif ticker == 'BTC': preco = preco_btc
        else: preco = preco_tesouro
        subtotal = qtd * preco
        total_geral += subtotal
        linhas_tabela.append({'Ativo': ticker, 'Classe': classe, 'Quantidade': qtd, 'Preço Atual': preco, 'Total Atual': subtotal})

df = pd.DataFrame(linhas_tabela)
df['Participação'] = (df['Total Atual'] / total_geral) if total_geral > 0 else 0
...
with aba_dash:
    ...
    with col1:
        ...
        fig_classe.update_traces(rotation=90, direction='counterclockwise')
...
with aba_detalhe:
    ...
    config_colunas = {
        'Ativo': st.column_config.TextColumn(alignment='center'),
        'Classe': st.column_config.TextColumn(alignment='center'),
        'Quantidade': st.column_config.NumberColumn(alignment='center'),
        'Preço Atual': st.column_config.NumberColumn(alignment='center', format="R$ %.2f"),
        'Total Atual': st.column_config.NumberColumn(alignment='center', format="R$ %.2f"),
        'Participação': st.column_config.NumberColumn(alignment='center', format="%.2f%%")
    }
...
with aba_novos_aportes:
    ...
    st.info('Aqui nós vamos programar os botões para salvar as compras mês a mês em um banco de dados real!')
